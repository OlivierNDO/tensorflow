...
with tf.Session() as sess:
    ...
    for epoch in range(n_epochs):
        ...
        train_acc = accuracy.eval(feed_dict={X: train_x_resized, y: train_y})
        val_acc = accuracy.eval(feed_dict={X: valid_x_resized, y: valid_y})
        train_loss = loss.eval(feed_dict={X: train_x_resized, y: train_y})
        val_loss = loss.eval(feed_dict={X: valid_x_resized, y: valid_y})
        #print(str(int(epoch + 1)), "Train Acc.:", train_acc, "Val Acc.:", val_acc, "Train Loss:", train_loss,"Val Loss:", val_loss)
